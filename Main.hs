...
main = hakyll $ do
    ...
    create ["feed/index.xml"] $ do
        ...
        compile $ do
            ...
            let
                ctx = mconcat
                    [ listField "posts" feedItemCtx (pure posts)
                    , constField "title" siteTitle
...
siteHost :: String
siteHost = "https://blog.restyled.io"

siteTitle :: String
siteTitle = "Restyled Blog"
...
feedItemCtx = mconcat
  ...
  where
    ...
    escapeXml = renderMarkup . toMarkup . NodeContent . pack
...
replace p f = pure . fmap (replaceAll p f)
